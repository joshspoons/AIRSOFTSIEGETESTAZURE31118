...
char[] menu1[]={"Search&Destroy","Sabotage","Domination", "Configuration"      };
  ...
  char[] SABOTAGE_FAIL= "ATTACKERS WIN";
...
void setup(){
  ...
  lcd.print("LAIDLAW &");// you can add your team name or someting cool
  lcd.setCursor(0,1);
  lcd.print(" SPOONS BOMB");// you can add your team name or someting cool
...
void keypadEvent(KeypadEvent key){
  switch (keypad.getState()){
    case RELEASED:
      switch (key){
         case '1': defusing=false;
         break;
         case '2': cancelando=false;
         break;
      }
    break;
    case HOLD:
      switch (key){
        case '1': defusing= true;
        break;
        case '2': cancelando=true;
        break;
      }
    break;
  }
...
void sabotage() {
  ...
  while (1) { // this is the important code, is a little messy but works good.
    ...
    //Code for led blinking
    timeCalcVar = (millis() - iTime) % 1000;
    if (timeCalcVar >= 0 && timeCalcVar <= 50)digitalWrite(BLUELED, HIGH);
    if (timeCalcVar >= 90 && timeCalcVar <= 130)digitalWrite(BLUELED, LOW);
